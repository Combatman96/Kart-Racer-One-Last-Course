...
public class CarAIHandler : MonoBehaviour
{
    ...
    void Start()
    {
        car.SetIsPlayer(false);
        SetFollowPath();
        // give the random perlin a random value
        m_RandomPerlin = Random.value * 100;
        m_Target = path[currentIndex];
        m_isDriving = true;
    }
    ...
    void Update()
    {
        ...
        {
            currentIndex++;
            currentIndex = Mathf.Clamp(currentIndex, 0, path.Count - 1);
            m_Target = path[currentIndex];
        }
    }
    ...
    bool IsBehindWaypoint()
    {
        return Vector3.Angle(path[currentIndex].forward, car.Position() - path[currentIndex].position) > 46f;
    }
    ...
    private void FixedUpdate()
    {
        if (!m_isDriving)
        {
            ...
            car.InputHandler(0, 0, false, false);
        }
        else
        {
            Vector3 fwd = transform.forward;
            if (m_Rigidbody.velocity.magnitude > car.MaxSpeed() * 0.1f)
            {
                ...
                case BrakeCondition.TargetDirectionDifference:
                    {
                        ...
                        desiredSpeed = Mathf.Lerp(car.MaxSpeed(), car.MaxSpeed() * m_CautiousSpeedFactor,
            ...
            float accelBrakeSensitivity = (desiredSpeed < car.GetSpeed())
                                              ? m_BrakeSensitivity
                                              : m_AccelSensitivity;

            // decide the actual amount of accel/brake input to achieve desired speed.
            float accel = Mathf.Clamp((desiredSpeed - car.GetSpeed()) * accelBrakeSensitivity, -1, 1);
            ...
            car.InputHandler(accel, steer, false, false);
            // Debug.Log("Car Input : " + accel + " " + steer);
